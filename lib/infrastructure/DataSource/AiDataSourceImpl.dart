...
class AiRemoteDataSourceImpl implements AiRemoteDataSource {
  ...
  Future<LoginResponseModel> login({LoginRequestModel? requestModel}) async {
    var result = await BaseBrain.dio
        .post(Api.LOGIN, data: requestModel?.toJson())
        .then((response) {
  ...
  @override
  Future<List<ChatResponseModel>> chatWithAi(
      {ChatRequestModel? requestModel}) async {
    var result = await BaseBrain.dio
        .post(
            requestModel?.maxTokens == 100
                ? Api.GENERATE_TRANSLATE
                : Api.GENERATE_AI_WRITER,
            data: requestModel?.toJson(),
            cancelToken: BaseBrain.cancelToken)
        .then((response) {
      List<dynamic> jsonList =
          (response.data as String).split('data:').map((jsonItem) {
        return jsonItem;
      }).toList();
      jsonList.removeAt(0);
